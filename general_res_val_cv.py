    ...
    for feat_sel in features_sel:
        ...
        for num in features_num:
            ...
            for i in range(cv_splits):
                ...
                (y_pred_pol, y_tr_pol, c_val_pol[i], gamma_val_pol[i],best_score_pol[i]) = cl.SVM_classify_poly_validation(train, y_train, test, y_true,
                C_range = np.logspace(-5, 2, 50),gamma_range = np.logspace(-6, 4, 50))
                (y_pred_lin, y_tr_lin, c_val_lin[i], best_score_lin[i]) = cl.SVM_classify_lin_validation(train, y_train, test, y_true,
